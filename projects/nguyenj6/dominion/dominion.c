...
int callMine(struct gameState *state, int choice1, int choice2, int handPos) {
    ...
    //***BUG***     if (choice2 > treasure_map || choice2 < curse) {
    if (choice2 > treasure_map && choice2 < curse) {
        return -1;
    }
    //***BUG***     if ((getCost(state->hand[currentPlayer][choice1]) + 10) > getCost(choice2)) {
    if ((getCost(state->hand[currentPlayer][choice1]) + 100) > getCost(choice2)) {
        return -1;
    }
...
//case baron
int callBaron(struct gameState *state, int choice1) {
    int currentPlayer = whoseTurn(state);
...
// case minion
int callMinion(struct gameState *state, int choice1, int choice2, int handPos) {
    int currentPlayer = whoseTurn(state);
    int i;
    int j;
    ...
    {
        //discard hand
        //***BUG***        while (numHandCards(state) > 0) {
...
// case ambassador
int callAmbassador(struct gameState *state, int choice1, int choice2, int handPos) {
    int currentPlayer = whoseTurn(state);
    int i;
    int j;
    ...
    //***BUG***     for (i = 0; i < state->handCount[currentPlayer]; i++) {
    for (i = 0; i <= state->handCount[currentPlayer]; i++) {
...
// case tribute
int callTribute(struct gameState *state, int choice1, int choice2, int handPos) {
    int currentPlayer = whoseTurn(state);
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
        ...
        if (state->deckCount[nextPlayer] == 0) {
            for (i = 0; i < state->discardCount[nextPlayer]; i++) {
                ...
                state->deckCount[nextPlayer]++;
                //***BUG*** state->discard[nextPlayer][i] = -1;
                state->discard[nextPlayer][i] = 0;
                state->discardCount[nextPlayer]--;
            }
...
//    int tributeRevealedCards[2] = {-1, -1};
    ...
    switch (card) {
        ...
        case mine:
            callMine(state, choice1, choice2, handPos);
        case tribute:
            callTribute(state, choice1, choice2, handPos);
        case ambassador:
            callAmbassador(state, choice2, choice1, handPos);
        case minion:
            callMinion(state, choice1, choice2, handPos);
        case baron:
            callBaron(state, choice1);
